...
class Sentence:
    ...
    def __contains__(self, item):
        for i in self._data:
            # If the item is an argument or the item is in the argument, then it is
            # in this
    ...
    def mapInto(self, other, replaceAll=True):
        """
        Returns a list of dicts of the smallest mapping of variables to sentences
        that if substituted
        into this sentence would make this sentence == other, or [] (empty list) if
        there is none
        ...
        # Check that other is not none and the aritys are the same for a quick sanity
        # check
        ...
        for m, n in zip(self, other):
            ...
            for mapping in m.mapInto(n):
                ...
                # If there is no mapping for a pair of arguments, then there is no
                # maping at all
                if not mapping:
                    continue
    ...
    def applyFunction(self, function, data=None):
        ...
        @param function - A function that takes 2 arguments, a sentence and a data
        object and returns a sentence
    ...
    def generalize(self):
        ...
        @return - a copy of this sentence with all of the literals as variables with
        the same name
        """
...
class Wff(Sentence):
    ...
    def __lt__(self, other):
        """
        A Wff is less than a sentence as long as the sentecne cannot be mapped into a
        Wff
        """
    ...
    def __eq__(self, other):
        """
        A Wff is equal to other if other is not a Wff or they share the same
        representation
        """
    ...
    def __ne__(self, other):
        """
        A Wff is not equal to other if other is a Wff and they dont share the same
        representation
        """
...
class Variable(Wff):
    ...
    def mapInto(self, other, replaceAll=True):
        ...
        """
        if len(other.args()) == 0 and (
            not isinstance(other, Wff) or isinstance(other, Variable)
        ):
            return [{self: other}]
        return []
