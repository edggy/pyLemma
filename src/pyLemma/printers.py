...
def prefixSentencePrinter(sen, symbols=None, sentencePrinter=None, isInfix=None):
    ...
    @param symbols - A dict with symbols to use.  Valid keys are: 'seperator',
    ...
    for arg in sen.args():
        string += (
            sentencePrinter(arg, symbols, sentencePrinter, isInfix)
            + symbols["seperator"]
        )
...
def infixSentencePrinter(sen, symbols=None, sentencePrinter=None, isInfix=None):
    ...
    if isInfix(sen):
        return (
            symbols["openParen"]
            + sentencePrinter(sen[1], symbols, sentencePrinter, isInfix)
            + symbols["space"]
            + str(sen.op())
            + symbols["space"]
            + infixSentencePrinter(sen[2], symbols, sentencePrinter, isInfix)
            + symbols["closeParen"]
        )
...
def defaultLinePrinter(line, sentencePrinter=None, howToPrint=None):
    ...
    ret = (
        str(lineNum)
        + "\t"
        + sentencePrinter(line._sentence, None, sentencePrinter, howToPrint)
    )
...
def defaultInferencePrinter(inf, sentencePrinter=None, howToPrint=None):
    """
    A printer that takes an inference and prints it in a easy to read, easy to parse
    format
...
def defaultProofPrinter(
    p,
    printedInferences=None,
    inferencePrinter=None,
    linePrinter=None,
    sentencePrinter=None,
):
    """
    A printer that takes a proof and prints it in a easy to read, easy to parse
    format with all the inference ruled used

    @param p - The  proof to be printed
    @param printedInferences - A set of inference rules, used to ensure the same rule
    ...
    # Print the word 'proof' to start the proof, and the name of the proof on the
    # next line
    res += "proof\n" + p.name + "\n"
    for n, line in enumerate(p):
        # For each line in the proof, set the line number to its appropriate number
        # and add it on a new line
        line._num = n
    ...
    if howToPrint is None:
        ...
        def howToPrint(sen):
            if sen.op() in notation:
                ...
                for arg in sen.args():
                    args.append(
                        sentencePrinter(arg, symbols, sentencePrinter, howToPrint)
                    )
...
def englishLinePrinter(line, sentencePrinter=None, howToPrint=None):
    ...
    if line._inference.name in howToPrint:
        ...
        lineFormat = "{0}) %s {1} %s {2} %s.\n" % (
            random.choice(nextPhrases),
            random.choice(inferencePhrases),
            support,
        )
...
def englishProofPrinter(
    ...
    for inf in inferences:
        ...
        if inf not in printedInferences:
            ...
            try:
                res += (
                    inferencePrinter(inferences[inf], sentencePrinter, howToPrint)
                    + "\n\n"
                )
            except AttributeError:
                res += (
...
def latexInferencePrinter(inf, sentencePrinter=None, howToPrint=None):
    ...
    for sen in inf._premises:
        assumptions += lineStructure % sentencePrinter(
            sen, sentencePrinter=sentencePrinter, howToPrint=howToPrint
        )
    for sen in inf.getConclusion():
        deductions += lineStructure % sentencePrinter(
            sen, sentencePrinter=sentencePrinter, howToPrint=howToPrint
        )
...
def latexSubproofPrinter(
    ...
    if not inline:
        ...
        return inferencesString + structure.format(
            assumptions[:-3], deductions[:-3], p.name
        )
    ...
    if len(supportLines) > 0:
        for supportLine in supportLines:
            if not inline or supportLine._data["length"] == 1:
                ...
                support += "%s-%s, " % (
                    numbering(supportLine._num),
                    numbering(supportLine._num + supportLine._data["length"] - 1),
                )
        # Delete the last ', '
        support = support[:-2]
...
def latexLinePrinter(line, sentencePrinter=None, howToPrint=None, inline=False):
    ...
    try:
        ...
        return (
            latexSubproofPrinter(
                line._inference,
                linePrinter=latexLinePrinter,
                sentencePrinter=sentencePrinter,
                howToPrint=howToPrint,
                linestart=line._num,
                inline=inline,
            )
            + "\n"
        ...
        if line._inference is not None:
            ...
            inf = str(line._inference.name)
            replacements = {
                "Contradiction Intro": r"$\bot$ Intro",
                "Contradiction Elim": r"$\bot$ Elim",
                "And Intro": r"$\land$ Intro",
                "And Elim Left": r"$\land$ Elim",
                "And Elim Right": r"$\land$ Elim",
                "Or Intro Left": r"$\lor$ Intro",
                "Or Intro Right": r"$\lor$ Intro",
                "Or Elim": r"$\lor$ Elim",
                "Not Intro": r"$\lnot$ Intro",
                "Not Elim": r"$\lnot$ Elim",
                "If Intro": r"$\lif$ Intro",
                "If Elim": r"$\lif$ Elim",
                "Iff Intro": r"$\liff$ Intro",
                "Iff Elim Left": r"$\liff$ Elim",
                "Iff Elim Right": r"$\liff$ Elim",
                "ForAll Intro": r"$\lall$ Intro",
                "ForAll Elim": r"$\lall$ Elim",
                "Exists Intro": r"$\lis$ Intro",
                "Exists Elim": r"$\lis$ Elim",
            }
...
"""
        ...
        deductions = ""
        for n, line in enumerate(p):
            # For each line in the proof, set the line number to its appropriate
            # number and add it on a new line
            line._num = n
