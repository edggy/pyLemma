...
class Proof:
    ...
    def setInference(self, inf, index=-1):
        """
        Set the inference rule for the line at **index**

        @param inf - The inference rule to add
        @param index - The line number to set the inference of
        """
        # TODO
        pass
    ...
    def addSupport(self, ref, index=-1):
        """
    ...
    def verify(self):
        """
        Verifies that the current proof is valid, i.e. each line validly follows from
        the previous lines

        @return - True if the proof is valid, otherwise the line number of the first
        invalid line
        """
        ...
        for line_num, line in enumerate(self._lines):
            ...
            # if there is no rule, then this line is not valid, unless the sentence
            # is also None (this is to allow empty lines)
            ...
            for ref in sup:
                try:
                    if ref()._num is None:
                        # This implies that ref refers to a line later in the proof (
                        # or itself), not an earlier one
                        err_line = line_num
                        break
            ...
            # Check that the sentence is a valid conclusion of the support steps
            # using thie given inference rule
    ...
    def makeMapping(self, conclusionMap, premises, references, exact=True):
        """
        Try to map all of the premises into the references in any combination while
        being constrained by the current conclusionMap
        ...
        @return - A map of subsitutions of variables in premises that will make
        premises match all the references
        """
    ...
    def makeMappingHelper(self, conclusionMap, premiseQueue, references):
        ...
        for curLine in references:
            ...
            for mapping in curPrem.mapInto(curLine.getSentence(), False):
                ...
                if (
                    "extra" in curPrem.extraData
                    and "newVars" in curPrem.extraData["extra"]
                ):
                ...
                # If the merge is successful recursively call makeMappingHelper for
                # the merged map, and the remainder of the premiseQueue
                remainder = self.makeMappingHelper(merge, premiseQueue, references)

                if len(remainder) == 0:
                    continue

                # If we can map the remaining premises, this is a valid mapping,
                # return it
                return remainder
    ...
    def __iadd__(self, value):
        ...
        """
        if (
            isinstance(value, inference.Inference)
            and value.name not in self._inferences
        ):
    ...
    def setNumbering(self, newNumbering):
        ...
        A numbering scheme should take a line number starting at 0 and return what it
        should look like
    ...
    def getPremises(self):
        ...
        for l in self._lines:
            inf = l.getInference()
            # An inference rule is an assumption iff it has no premises, has exactly
            # one conclusion,
            ...
            # A
            if (
                len(inf.getPremises()) == 0
                and len(inf.getConclusion()) == 1
                and inf.getConclusion()[0] <= sentence.Variable()
            ):
    ...
    def getConclusion(self):
        ...
        return list(map(lambda a: a.getSentence(), self._lines))
